...
export async function getClass() {
    return await sendMessage("getClass", null);
}

// 提交评语
export async function submitComments(data) {
    return await sendMessage("submitComments", data);
}
...
// 保存评语模板
export async function saveCommentTemplate(data) {
    try {
        if (data) {
            //localStorage.setItem("template", JSON.stringify(data));
            await chrome.storage.local.set({ template: data })
            return true;
        } else {
            return false;
        }
...
// 获取评语模板
export async function getCommentTemplate() {
    //return JSON.parse(localStorage.getItem("template"));
    let { template } = await chrome.storage.local.get(['template']);
    return template;
}

// 刷新Content
export async function refreshContent() {
    await sendMessage("refreshContent", null);
}
